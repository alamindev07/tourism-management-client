...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      // Build user object to save to DB with role "tourist"
      const user = {
        name,
        email,
        photoURL: photo || "", 
  ...
  //     // Save Google user to DB with default role "tourist"
  //     const userToSave = {
...
const handleGoogleSignIn = async () => {
  ...
  try {
    const result = await signInWithGoogle(); // get the full result
    const user = result.user; // get user from result

    if (!user) throw new Error("No user returned from Google sign-in");

    // Prepare user object to save to DB with default role "tourist"
    const userToSave = {
      name: user.displayName || "Google User",
      email: user.email,
      photoURL: user.photoURL || "",
    ...
    await saveUserToDB(userToSave); // Save user to backend DB
